...
export default new NativeFunction({
  ...
  async execute(ctx) {
    ...
    const stats = (ctx.client as any).rateLimitStats || new Map();
    ...
    surgeStats.queueWaitTimes = surgeStats.queueWaitTimes.filter((entry: any) => now - entry.time < 5 * 60 * 1000);

    // Calculate P95 wait time
    const waitTimes = surgeStats.queueWaitTimes.map((entry: any) => entry.wait);
    waitTimes.sort((a: number, b: number) => a - b);
