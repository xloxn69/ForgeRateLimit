...
export default new NativeFunction({
  ...
  async execute(ctx) {
    ...
    const flowJobs = new Map<string, any[]>();
    ...
    for (const [flowId, count] of flowCounts) {
      const processedCount = currentRuns.filter((run: any) => run.flowId === flowId).length || 0;
    ...
    const jobIndex = queue.findIndex((job: any) => job.id === selectedJob.id);
    if (jobIndex !== -1) {
