...
export class ForgeRateLimit extends ForgeExtension {
  ...
  private eventCommands: Map<string, any> = new Map();
  ...
  private setupEventListeners(client: ForgeClient) {
    const events = ['tokenReserved', 'throttled', 'queued', 'circuitBreaker', 'surgeGuard', 'bucketRefilled', 'tokenRefunded', 'queueExecuted', 'policyChanged'];
    
    events.forEach(eventName => {
      this.emitter.on(eventName as any, (data: any) => {
        ...
        if (command) {
          ...
          Interpreter.run(mockContext).catch((err: any) => {
